...
char** GetArgs(int *pArgc, eDpdkAppType appType)
{
	...
	if (pArgc) {
		...
		if (appType == DPDK_APP_TESTPMD) {
			...
			strncpy(&myArgsArray[argc++][0], "7-10", DPDK_ARGS_MAX_ARG_STRLEN-1);

			strncpy(&myArgsArray[argc++][0], "--master-lcore", DPDK_ARGS_MAX_ARG_STRLEN-1);
			strncpy(&myArgsArray[argc++][0], "7", DPDK_ARGS_MAX_ARG_STRLEN-1);
			...
			strncpy(&myArgsArray[argc++][0], "--forward-mode=io", DPDK_ARGS_MAX_ARG_STRLEN-1);
			strncpy(&myArgsArray[argc++][0], "--txq=3", DPDK_ARGS_MAX_ARG_STRLEN-1);
			strncpy(&myArgsArray[argc++][0], "--rxq=3", DPDK_ARGS_MAX_ARG_STRLEN-1);
